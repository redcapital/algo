...
int getsize(Node* n) {
	...
	return n->totalsize = sz;
}

void find_right_dir(Node* n) {
	if (n->totalsize >= need_to_free) {
		ans = min(ans, n->totalsize);
	}
	for (auto c: n->children) {
		if (!c->size) {
			find_right_dir(c);
...
int main() {
  ...
  getsize(&root);
  int remaining = SPACE_TOTAL - root.totalsize;
  need_to_free = SPACE_REQUIRED - remaining;
  ans = root.totalsize;
  find_right_dir(&root);
  PRINTL(ans);
  return 0;
